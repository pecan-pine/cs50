...
def create_figure():
    ...
    # get list of all foods the user ate
    foodlist = [food["food_item"] for food in db.execute("SELECT DISTINCT \
                        food_item FROM foods WHERE user_id=:user_id", user_id = user_id)]
    ...
    for food in foodlist:
        ...
        x = [time["date"] for time in db.execute("SELECT date FROM \
                    foods WHERE user_id=:user_id AND food_item=:food_item ORDER BY date", user_id = user_id, food_item = food)]
    ...
    feelinglist = db.execute("SELECT feeling, date FROM feelings WHERE user_id=:user_id ORDER BY date", user_id = user_id)
...
def eat():
    ...
    if request.method == "POST":
        ...
        if date:
            ...
            date = datetime.now()
        # get the username by querying the database
...
def record():
    ...
    if request.method == "POST":
        ...
        # TODO: make username a global variable instead of looking for it a bunch of times
