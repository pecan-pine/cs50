...
def create_figure():
    ...
    req = Request(session["mango_url"], headers={'User-Agent': 'Mozilla/5.0'})
    f = urlopen(req)
    # read the image file in a numpy array
    a = plt.imread(f, 0)
    im2 = OffsetImage(a, zoom=.5)
    time = dt.datetime.now() -dt.timedelta(days=2)
    ab2 = AnnotationBbox(im2, (time,5), frameon=False)

    req = Request(session["fruit_url"], headers={'User-Agent': 'Mozilla/5.0'})
    f = urlopen(req)
    # read the image file in a numpy array
    a = plt.imread(f, 0)
    im = OffsetImage(a, zoom=.5)
    time = dt.datetime.now() -dt.timedelta(days=1)
    ab = AnnotationBbox(im, (time,5), frameon=False)
    subsub = fig.add_subplot(1,1,1, sharex=sub, sharey=sub)
    subsub.patch.set_alpha(0)
    subsub.axis("off")
    subsub.add_artist(ab)
...
def login():
    ...
    if request.method == "POST":
        ...
        session["d"] = dt.timedelta(hours=1.5)
