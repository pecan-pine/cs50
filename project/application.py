...
def n(food, low_feeling_dict):
    ...
    while d == session["d"]: #d <= dt.timedelta(days=1): #<-to make it loop over different d # d == session["d"]: # to make one value again
...
def create_figure():
    ...
    for food in session["food_list"]:
        ...
        # if the food has a url and the number of foods is less than 10, display pictures
        if session[f"{food}_url"]: #and len(session["food_list"]) < 10:
            ...
            for x0, y0 in zip(x,y):
                ...
                p = sub.plot([x0, x0+session["d"]],[y0,y0], "-o")
                color = p[0].get_color()
                # draw wiggly lines
                rcParams['path.sketch'] = (5, 4, 2)

                # draw the epsilon interval
                sub.plot([x0+session["d"]-session["epsilon"], x0+session["d"]+session["epsilon"]], [y0,y0], color=color)

                # draw normal lines again
                matplotlib.rcdefaults()
        # if there are too many foods, just plot dots
        # TODO: make squiggly lines appear in this case
        else:
            # plot the food using a dot
            sub.plot(x,y,"o", label=food)
            # add a label under the dot
            #for x0, y0 in zip(x,y):
            #    sub.annotate(food, (x0-dt.timedelta(hours=3),y0-.5), alpha=1, backgroundcolor="white")
...
def eat():
    ...
    if request.method == "POST":
        ...
        if food in session["food_list"]:
            ...
            session[f"{food}_url"] = picture(food)
            # this will be slightly different (decimal seconds) than the version loaded into session variable
            session[food] = [date]
...
def login():
    ...
    if request.method == "POST":
        ...
        session["epsilon"] = dt.timedelta(hours=2)
        ...
        # in the form of a list of str(datetime) objects in PDT time
