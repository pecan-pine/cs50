...
def buy():
    """Buy shares of stock"""
    # open file containing some stock symbols (from wikipedia)
    # put stock symbols into a list for the buy.html to read
    # a datalist will suggest the common symbols to the user
    with open("static/symbol_list.csv", "r") as file:
        example_symbols = [line.rstrip() for line in file.readlines()]
    if request.method == "POST":
        ...
        return render_template("buy.html", example_symbols=example_symbols)
...
def history():
    ...
    # Symbol Buy/Sold Shares Price Date '''
    purchases = db.execute("SELECT stock_symbol as symbol, \
                        num_shares as shares, \
                        share_value as price, \
                        purchase_date as date \
                        FROM purchases WHERE user_id=:user_id", user_id = session["user_id"])
...
def quote():
    ...
    if request.method=="POST":
        ...
        if not lookup(symbol) or type(lookup(symbol))==None:
...
def sell():
    ...
    if request.method == "POST":
        ...
        # today's date/time for the purchase/sale record
        date = datetime.now()
        # find the current price of the stock
        price = lookup(symbol)["price"]
        # make a database table entry for the sale
        # entered in just like a purchase but with negative shares
        db.execute("INSERT INTO purchases \
            (user_id, stock_symbol, purchase_date, share_value, num_shares) \
            VALUES (:user_id, :symbol, :date, :price, :shares)", \
        ...
        cash = db.execute("SELECT cash FROM users WHERE id=:user_id", user_id=user_id)[0]["cash"]
        # find the value of the shares to be sold, then round
        value = round(price * shares_requested, 2)
        # new balance is the user's cash plus the value of the sale
        balance = value + cash
        # update users cash balance
        db.execute("UPDATE users SET cash=:balance WHERE id=:user_id", user_id=user_id, balance=balance)
        return redirect("/")
